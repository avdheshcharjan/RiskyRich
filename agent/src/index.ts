...
// We'll load telegramPlugin dynamically later
import postgresPlugin from "@elizaos/adapter-postgres";
        ...
        const importedPlugins = await Promise.all(
            plugins.map(async (plugin) => {
                try {
                    ...
                    const moduleExport = importedPlugin as Plugin & { default?: Plugin };
                    if (!moduleExport[functionName] && !moduleExport.default) {
                      elizaLogger.warn(plugin, 'does not have an default export or', functionName)
                    }
                    return {...(
                        moduleExport.default || moduleExport[functionName]
                    ), npmName: plugin };
                } catch (importError) {
                    console.error(
...
export async function createAgent(
    ...
    // Try to load the telegram plugin
    try {
        // Try different approaches to import
        const telegramPath = process.env.TELEGRAM_PLUGIN_PATH || '@elizaos-plugins/client-telegram';
        
        // First try npm package
        let telegramModule;
        try {
            telegramModule = await import(telegramPath);
        } catch (e) {
            // Then try local path
            try {
                telegramModule = await import('../../packages/client-telegram/dist/index.js');
            } catch (e2) {
                throw new Error(`Failed to import telegram plugin: ${e.message}, ${e2.message}`);
            }
        }
        
        // The plugin could be exported as default or as telegramPlugin
        const plugin = telegramModule.default || telegramModule.telegramPlugin;
        if (plugin) {
            elizaLogger.info("Successfully loaded telegram plugin");
            plugins.push(plugin);
        } else {
            elizaLogger.warn("Telegram plugin loaded but no plugin export found");
        }
    } catch (error) {
        elizaLogger.warn(`Could not load telegram plugin: ${error.message}`);
    }
